...
typedef struct {
	int fila, col; // Origen de la ficha seleccionada
	tPosicion posicion; // sentido
} tMovimiento;
...
typedef struct {
	...
	int posMov = 0; //Contador en la lista de movimientos.
} tJuego;
...
bool posiblesMovimientosInversos(const tJuego &juego, tMovimiento mov, tPosibles &posibles); // MIO
void movimientoInversoAleatorio(const tJuego &juego, const tPosibles &posible, tMovimiento &mov);
void ejecutarMovimientoInverso(tJuego &juego, tMovimiento mov); // MIO
