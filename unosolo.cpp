...
bool cargar(tJuego &juego, string nombre) {
	...
	if (fichero.is_open()) {
		fichero >> dimTxt;
		if (dimTxt == DIM) { //Tiene que coincidir la dimension en archivo y programa.
			fichero >> juego.bolasIniciales;
			fichero >> juego.filaMeta;
			fichero >> juego.colMeta;
			for (int fila = 0; fila < DIM; fila++) //Carga el tablero.
	...
	do {
		...
		if (leer) { // Si el movimiento se ha leido correctamente.
			if (!deshacer) {
				...
				cout << "lista: " << juego.numMovimientos - 1 << "[" << mov.fila << "] [" << mov.col << "] " << mov.posicion << endl;
			}
			else {
...
/* Posiciones del enumerado tPosicion:
------------------------------------------------------|
1º ARRIBA						  |
3ºIZQUIERDA								  4ºDERECHA   |
2º ABAJO						  |
------------------------------------------------------|
*/
...
bool leerMovimiento(const tJuego juego, tMovimiento &mov, bool &deshacer){
	...
	do {
		salir = selecFicha(juego, mov, valida, deshacer); // Se selecciona una ficha, se sale o se deshace movimientos.
		if (!salir && !deshacer) {
			if (valida) { // Si la casilla seleccionada es una ficha.
				puedeMover = posiblesMovimientos(juego, mov, posible);
				if (puedeMover) { //La ficha tiene al menos un movimiento disponible.
					...
					if (opcion != 0) {
						...
						if (!leer) { // La direccion escogida no existe por las siguientes razones:
							if (opcion == 1)
				...
				else // La ficha no se puede mover.
					cout << "La ficha seleccionada no se puede mover. Intentalo de nuevo" << endl;
			}

			else { // Si la casilla seleccionada no es ficha, quedan tres opciones:
...
bool posiblesMovimientos(const tJuego &juego, tMovimiento mov, tPosibles &posible){
	...
	for (int i = 0; i < NUM_DIRS; i++) {
		if (i == 0) {
			if (mov.fila - 2 >= 0) { //Para no salirse del rango arriba.
...
int mostrarMovimiento(const tPosibles &posible) {
	...
	for (int i = 0; i < NUM_DIRS; i++)
		if (posible[i]) {  //Solo se mostrara los movimientos posibles.
			if (i == 0)
...
void deshacerMovimientos(tJuego &juego) {
	...
	cout << "La posicion en la lista de Mov(antes bucle) es: " << juego.posMov << endl;
	...
	juego.posMov++; // Lleva a la posicion actual de lista movimientos.
}

bool jugando(tJuego juego) {
	//Comprueba que haya algun movimiento disponible.
	bool jugar = false;
	bool mover;
...
bool posiblesMovimientosInversos(const tJuego &juego, tMovimiento mov, tPosibles &posible){
	...
	cout << "Puede mover es: " << mover << endl;
...
void movimientoInversoAleatorio(const tJuego &juego, const tPosibles &posible, tMovimiento &mov) {
	...
	cout << "Ficha [" << mov.fila << "] [" << mov.col << "]" << endl;
	cout << "Direccion: " << pos << endl;
